...
function fillTable(user) {
    ...
    for (var r = 0; r < size; r += 1) {
        var record = document.getElementById("insertApproverRei").insertRow(r);

        for (var c = 0; c < 6; c += 1) {
            var y = record.insertCell(c);
        }
        var table = document.getElementById("insertApproverRei"),
            reID = user[r].rei_id,
            empID = user[r].emp_id,
            lastName = user[r].awardRequested,
            eventSubmission = user[r].eventStartDate,
            eventStatus = user[r].rei_state;

        table.rows[r].cells[0].innerHTML = reID;
        table.rows[r].cells[1].innerHTML = empID;
        table.rows[r].cells[2].innerHTML = lastName;
        table.rows[r].cells[3].innerHTML =eventSubmission;
        table.rows[r].cells[4].innerHTML = eventStatus;
        table.rows[r].cells[5].innerHTML = '<form action="Reviewing.do" method="POST"><input type="hidden" id="name" name="name" value="' +
            reID +
            '"><input class="table-button-only" type="submit" name="Rev' +
            reID + '" value="Review"></form>';
    }
}
