...
public class StringReverse {
	...
	public static String reverse(String s) {
		...
		// converts the input string into a character array as String's are immutable in Java :(
		char[] charArr = s.toCharArray();
		
		// Loops through the first half of the string, swapping characters with 
		for (int i = 0; i < (int) (charArr.length / 2); i++) {
			
			// XOR Swap of corresponding characters in the char array, using their ASCII values
			charArr[i] = (char) (charArr[i] ^ charArr[charArr.length - i - 1]);
			charArr[charArr.length-i - 1] = (char) (charArr[i] ^ charArr[charArr.length - i - 1]);
			charArr[i] = (char) (charArr[i] ^ charArr[charArr.length - i - 1]);
		}
		
		// new String instantiated with the character Array that now contains the reversed String
